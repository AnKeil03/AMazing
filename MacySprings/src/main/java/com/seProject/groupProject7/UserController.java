...
public class UserController {
    ...
    @RequestMapping(value = "/admin/promoteuser")
    public String promote(@CookieValue(value = "sessionID", defaultValue = "-1") String cval, @RequestParam String userPromote) {
        System.out.println("access from "+cval);
        User U = getUserForID(Integer.parseInt(cval));
        User U2 = getUser(userPromote);
        if (U!=null) {
            if (U2!=null) {
                if (U.getPrivileges() == User.PRIVILEGES_ADMIN) {
                    U2.setPrivileges(User.PRIVILEGES_ADMIN);
                    userRepository.save(U2);
                    return "user " + userPromote + " promoted to admin";
                } else {
                    return "you need admin privileges to use this function.";
                }
                //return "Your user info:\nName:" + U.getName() + "\nEmail:\n" + U.getEmail();
            } else {
                return "attempting to modify nonexistent user";
            }
        }
        else
            return "please login";
